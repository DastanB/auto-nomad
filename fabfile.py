...
prod_manager_ip = os.getenv('PROD_ADDR')
...
@hosts([prod_manager_ip])
def prod():
    env.hosts = [prod_manager_ip]
    env.branch = os.getenv('PROD_GIT_BRANCH')
    env.app_path = os.getenv('PROD_APP_PATH')
    ...
    env.registry_password = os.getenv('CI_REGISTRY_PASSWORD')
    env.commit_sha = os.getenv('CI_COMMIT_SHA')
    env.compose_file = 'prod.yml'
    env.stack_name = os.getenv('PROD_STACK_NAME')
...
def docker_stack_deploy(exports):
    run('%s; cd %s; docker stack deploy -c %s --with-registry-auth %s' % (exports, env.app_path, env.compose_file, env.stack_name))
